...
def dsc_loss(logits, label, gamma=0., name=None):
    y_pred = fluid.layers.softmax(logits, axis=1)
    label = fluid.layers.one_hot(label, depth=logits.shape[-1])
    dice_numerator = 2 * (1.0 - y_pred) * y_pred * label + gamma
    dice_denominator = (1.0 - y_pred) * y_pred + label + gamma
    dice_score = 1.0 - dice_numerator / dice_denominator
